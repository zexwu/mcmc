...
PYBIND11_MODULE(VBBinaryLensing, m) {
    ...
    py::class_<VBBinaryLensing> vbb(m, "VBBinaryLensing");
       ...
       vbb.def("ESPLMag2", &VBBinaryLensing::ESPLMag2,
        ...
        vbb.def("BinaryMag2_vec",
            [](VBBinaryLensing &self, double s, double q, std::vector<double> y1s, std::vector<double> y2s, double rho)
            {
                std::vector<double> mags(y1s.size());
                self.BinaryMag2_vec(s, q, mags.data(),
                    y1s.data(), y2s.data(), rho, mags.size());
                return mags;
            },
            R"mydelimiter(
            Static binary lens light curve for a given set of parameters.
